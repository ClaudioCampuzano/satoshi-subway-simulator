...
class subwayNetwork():
    ...
    def generateNetwork(self, fileNetwork):
        ...
        try:
            ...
            self._networkRed = self.__generateCustomNetwork(
                networkTopololy['greenStation'])
            self._networkGreen = self.__generateCustomNetwork(
                networkTopololy['redStation'])
    ...
    def __generateCustomNetwork(self, specialStations):
        ...
        for specialStation in specialStations.strip().split(','):
            ...
            for newInterconnection in newInterconnections:
                networkCopy[newInterconnection].update(
                    newInterconnections-set(newInterconnection))
        return networkCopy
    ...
    def findShortestPath(self, initialStation, endStation, trainColor):
        ...
        if trainColor == 'green':
            networkUse = self._networkGreen
        elif trainColor == 'red':
            networkUse = self._networkRed
        else:
            networkUse = self._networkWhite

        if len(networkUse) == 0:
            return('Empty path, apparently the topology file was not loaded')
        ...
        if initialStation == endStation:
            return("Station starts and ends the same")
...
if __name__ == '__main__':
    ...
    trainColor = input(
        'Enter train color (red, green or press enter to skip): ')
